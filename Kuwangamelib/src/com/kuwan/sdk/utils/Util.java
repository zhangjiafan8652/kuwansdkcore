...
public final class Util {
	...
	public static String getGameId(Context paramContext) {
		...
		if (((DataInfo = getMetaDataInfo(paramContext)) == null)
				|| (DataInfo.get("kuwan_game_id") == null)) {
			throw new IllegalArgumentException("must set the kuwan_game_id");
		}
		return DataInfo.getString("kuwan_game_id").replace("yaya", "");
	}

	public static String getSourceId(Context paramContext) {
		Bundle DataInfo;
		if (((DataInfo = getMetaDataInfo(paramContext)) == null)
				|| (DataInfo.get("kuwan_source_id") == null)) {
			return null;
		}
		return DataInfo.get("kuwan_source_id").toString();
	}
	...
	public static String getSourceId1(Context paramContext) {
		Bundle DataInfo;
		if (((DataInfo = getMetaDataInfo(paramContext)) == null)
				|| (DataInfo.get("kuwan_game_key") == null)) {
			return null;
		}
		return DataInfo.get("kuwan_game_key").toString();
	}
